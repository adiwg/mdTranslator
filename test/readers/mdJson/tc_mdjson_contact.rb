...
class TestReaderMdJsonContact < TestReaderMdJsonParent
   ...
   def test_contact_schema
      ...
      errors = TestReaderMdJsonParent.testSchema(hIn[0], 'contact.json', remove: ['externalIdentifier'])

      assert_empty errors

      # test is organization
      hIn = Marshal::load(Marshal.dump(@@mdHash))
      errors = TestReaderMdJsonParent.testSchema(hIn[1], 'contact.json', :remove => ['positionName', 'externalIdentifier'])
      assert_empty errors
