...
# readers / fgdc / module_horizontalPlanar / lambert azimuthal equal area projection
...
class TestReaderFgdcPlanarLambertEqualArea < TestReaderFGDCParent
   ...
   def test_planar_lambertEqualArea
      ...
      xIn = @@xDoc.xpath('./metadata/spref/horizsys/planar[7]')
      ...
      assert_equal 600000, hProjection[:falseNorthing]
      assert_equal 'feet', hProjection[:falseEastingNorthingUnits]

      hProjectionId = hProjection[:projectionIdentifier]
      assert_equal 'lambertEqualArea', hProjectionId[:identifier]
      assert_equal 'Lambert Azimuthal Equal Area', hProjectionId[:name]
