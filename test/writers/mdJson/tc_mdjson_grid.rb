...
class TestWriterGrid < TestWriterMdJsonParent
   ...
   def test_complete_grid
      ...
      expect = expect['metadata']['resourceInfo']['spatialRepresentation'][0]['gridRepresentation']
      got = JSON.parse(metadata[:writerOutput])
      got = got['metadata']['resourceInfo']['spatialRepresentation'][0]['gridRepresentation']
