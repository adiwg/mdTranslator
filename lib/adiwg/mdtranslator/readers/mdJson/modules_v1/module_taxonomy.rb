...
require ADIWG::Mdtranslator::Readers::MdJson.readerModule('module_voucher')
require ADIWG::Mdtranslator::Readers::MdJson.readerModule('module_taxonClass')
                    ...
                    def self.unpack(hTaxonomy, responseObj)
                        ...
                        if hTaxonomy.has_key?('classificationSystem')
                            ...
                            unless aClassSys.empty?
                                aClassSys.each do |hCitation|
                                    intTaxSys[:taxClassSys] << Citation.unpack(hCitation, responseObj)
                                end
                            end
                        end
                        ...
                        if hTaxonomy.has_key?('observer')
                            ...
                            unless aObservers.empty?
                                aObservers.each do |observer|
                                    intTaxSys[:taxObservers] << ResponsibleParty.unpack(observer, responseObj)
                                end
                            end
                        end
                        ...
                        if hTaxonomy.has_key?('voucher')
                            ...
                            unless hVoucher.empty?
                                intTaxSys[:taxVoucher] = Voucher.unpack(hVoucher, responseObj)
                            end
                        end
                        ...
                        if hTaxonomy.has_key?('taxonClass')
                            ...
                            unless aTaxClass.empty?
                                aTaxClass.each do |hTaxClass|
                                    intTaxSys[:taxClasses] << TaxonCl.unpack(hTaxClass, responseObj)
                                end
                            end
                        end
